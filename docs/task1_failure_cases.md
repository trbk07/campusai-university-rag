...
| Multi-page tables | Table may be split per page or continuation rows may be misclassified | Only consecutive page tables with identical normalized column names are stitched; a repeated first row matching the previous header is dropped; non-adjacent or differing schemas remain separate |
